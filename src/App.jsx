...
function IntroScreen({ onStart }) {
  ...
  const inp = { width:"100%", padding:"11px 14px", fontSize:14, borderRadius:10, boxSizing:"border-box", border:"1.5px solid #E5E7EB", outline:"none", fontFamily:"inherit", color:BRAND.black, background:"#fff" };
  return (
    <div style={{ maxWidth:620, margin:"0 auto", padding:"48px 20px 48px" }}>
      <div style={{ marginBottom:32 }}>
        ...
        <h1 style={{ fontSize:34, fontWeight:700, color:BRAND.black, lineHeight:1.15, margin:"0 0 10px", fontFamily:"'Newsreader','Iowan Old Style',Georgia,serif" }}>Self-assessment<br/>levelling exercise</h1>
      ...
      <div style={{ background:"#FAFAFA", border:"1px solid #E5E7EB", borderRadius:14, padding:"20px 22px", marginBottom:24 }}>
        ...
        <div>
          <label style={{ display:"block", fontSize:12.5, fontWeight:600, color:"#374151", marginBottom:6 }}>Your manager's email <span style={{ fontSize:11, fontWeight:400, color:"#9CA3AF" }}>(optional)</span></label>
          <input type="email" placeholder="e.g. manager@saas.group" value={managerEmail} onChange={e => setManagerEmail(e.target.value)} style={inp} onFocus={e => (e.target.style.borderColor=BRAND.cinnabar)} onBlur={e => (e.target.style.borderColor="#E5E7EB")} />
          <p style={{ fontSize:11.5, color:"#9CA3AF", margin:"5px 0 0" }}>We'll include this in the sharing instructions on your results page.</p>
        </div>
      </div>
      ...
      <button onClick={() => canStart && onStart(name.trim(), memberEmail.trim(), managerEmail.trim())} disabled={!canStart}
        ...
        onMouseLeave={e => { if(canStart) e.currentTarget.style.background=BRAND.cinnabar; }}>
...
function PhaseTransitionScreen({ onContinue }) {
  return (
    <div style={{ maxWidth:620, margin:"0 auto", padding:"64px 20px 48px", textAlign:"center" }}>
      ...
      <button onClick={onContinue} style={{ width:"100%", padding:"15px 24px", fontSize:15, fontWeight:600, color:"#fff", background:BRAND.indigo, border:"none", borderRadius:12, cursor:"pointer", transition:"background 0.2s" }}
  ...
  return (
    <div ref={wrapRef} style={{ maxWidth:620, margin:"0 auto", padding:"28px 20px 40px" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", marginTop:28, gap:12 }}>
        <button onClick={onPrev} disabled={isFirst} style={{ padding:"11px 22px", fontSize:13.5, fontWeight:600, borderRadius:10, border:"1px solid #E5E7EB", background:"#fff", color:isFirst?"#D1D5DB":BRAND.black, cursor:isFirst?"default":"pointer" }}>Back</button>
...
function ResultsScreen({ answers, memberName, memberEmail, managerEmail, onRestart }) {
  ...
  const calcThemeResults = themes => themes.map(theme => {
  ...
  const downloadPdf = () => {
    ...
    const rows = allSections.map(({ label, results }) =>
      `<tr><td colspan="3" style="padding:14px 16px 6px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#9CA3AF;border-top:2px solid #E5E7EB;">${label}</td></tr>` +
      results.flatMap(({ theme, scores }) =>
        scores.map((s,i) =>
          `<tr>${i===0?`<td rowspan="${scores.length}" style="padding:10px 12px;vertical-align:top;font-weight:700;font-size:11px;color:#fff;background:${theme.color};white-space:nowrap;line-height:1.3;">${theme.name}</td>`:""}` +
          `<td style="padding:9px 14px;font-size:12.5px;color:#374151;border-bottom:1px solid #F3F4F6;">${s.name}</td>` +
          `<td style="padding:9px 14px;text-align:center;border-bottom:1px solid #F3F4F6;"><span style="display:inline-block;padding:2px 10px;border-radius:20px;background:${theme.color}18;color:${theme.color};font-size:11px;font-weight:700;">${s.level||"—"}</span><div style="font-size:10px;color:#9CA3AF;margin-top:2px;">${LEVEL_LABELS[s.level]||""}</div></td></tr>`
        )
      ).join("")
    ).join("");

    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${memberName} — Self-Assessment</title>
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,'Segoe UI',Arial,sans-serif;background:#F3F4F6;padding:32px 16px}.wrap{max-width:700px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,0.08)}.hdr{background:${BRAND.cinnabar};padding:26px 32px}.hdr .tag{font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:rgba(255,255,255,0.7);margin-bottom:6px}.hdr h1{font-size:20px;font-weight:700;color:#fff;margin-bottom:4px}.hdr .sub{font-size:12px;color:rgba(255,255,255,0.8)}.sum{display:flex;margin:24px 32px 16px;background:linear-gradient(135deg,${BRAND.cinnabarTint},${BRAND.indigoTint});border-radius:12px;border:1px solid #E5E7EB;overflow:hidden}.sum .ov{flex:0 0 36%;text-align:center;padding:18px;border-right:1px solid #E5E7EB}.sum .ov .lbl{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:#6B7280;margin-bottom:4px}.sum .ov .lvl{font-size:40px;font-weight:800;color:${BRAND.black};line-height:1}.sum .ov .nm{font-size:12px;color:#374151;font-weight:500;margin-top:3px}.sum .ov .av{font-size:10px;color:#9CA3AF;margin-top:3px}.sum .pts{flex:1;padding:14px;display:flex;flex-direction:column;gap:7px}.sum .pt{background:#fff;border-radius:7px;padding:9px 12px;border:1px solid #E5E7EB}.sum .pt .ptg{font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;margin-bottom:2px}.sum .pt .pl{font-size:15px;font-weight:800;color:${BRAND.black}}.sum .pt .pn{font-size:10px;color:#6B7280;margin-left:4px}.tip{margin:0 32px 16px;background:#FFFBEB;border:1px solid #FDE68A;border-radius:8px;padding:9px 12px;font-size:11.5px;color:#92400E}table{width:calc(100% - 64px);margin:0 32px 24px;border-collapse:collapse}.ftr{background:#F9FAFB;padding:12px 32px;border-top:1px solid #E5E7EB;font-size:10px;color:#9CA3AF}@media print{body{background:#fff;padding:0}.wrap{box-shadow:none;border-radius:0}.tip{display:none}}</style>
...
<div class="pts"><div class="pt"><div class="ptg" style="color:${BRAND.cinnabar};">Part 1 · General</div><span class="pl">${generalLevel}</span><span class="pn">${LEVEL_LABELS[generalLevel]} · avg ${generalAvg.toFixed(1)}</span></div>
<div class="pt"><div class="ptg" style="color:${BRAND.indigo};">Part 2 · Accounting</div><span class="pl">${functionalLevel}</span><span class="pn">${LEVEL_LABELS[functionalLevel]} · avg ${functionalAvg.toFixed(1)}</span></div></div></div>
<div class="tip">💡 To save as PDF: <strong>File → Print → Save as PDF</strong> (or <strong>Cmd+P</strong> / <strong>Ctrl+P</strong>)</div>
  ...
  const buildEmailHtml = () => {
    const themeRows = results => results.map(({ theme, scores, avg, dominant }) =>
      `<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:10px;border-radius:10px;overflow:hidden;border:1px solid #E5E7EB;">
        <tr><td style="background:${theme.color};padding:10px 16px;"><span style="color:#fff;font-weight:700;font-size:13px;">${theme.name}</span><span style="color:rgba(255,255,255,0.8);font-size:11px;float:right;padding-top:1px;">${dominant} · avg ${avg.toFixed(1)}</span></td></tr>
...
<td width="38%" style="padding:20px;text-align:center;border-right:1px solid #E5E7EB;"><div style="font-size:10px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:6px;">Overall indicative level</div><div style="font-size:44px;font-weight:800;color:${BRAND.black};line-height:1;">${overallLevel}</div><div style="font-size:13px;color:#374151;font-weight:500;margin-top:4px;">${LEVEL_LABELS[overallLevel]}</div><div style="font-size:11px;color:#9CA3AF;margin-top:4px;">avg ${overallAvg.toFixed(1)}</div></td>
<td style="padding:16px 18px;"><div style="background:#fff;border-radius:8px;padding:10px 14px;margin-bottom:8px;border:1px solid #E5E7EB;"><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:${BRAND.cinnabar};margin-bottom:3px;">Part 1 · General</div><span style="font-size:17px;font-weight:800;color:${BRAND.black};">${generalLevel}</span><span style="font-size:11px;color:#6B7280;margin-left:5px;">${LEVEL_LABELS[generalLevel]} · avg ${generalAvg.toFixed(1)}</span></div><div style="background:#fff;border-radius:8px;padding:10px 14px;border:1px solid #E5E7EB;"><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:${BRAND.indigo};margin-bottom:3px;">Part 2 · Accounting</div><span style="font-size:17px;font-weight:800;color:${BRAND.black};">${functionalLevel}</span><span style="font-size:11px;color:#6B7280;margin-left:5px;">${LEVEL_LABELS[functionalLevel]} · avg ${functionalAvg.toFixed(1)}</span></div></td>
</tr></table></td></tr>
<tr><td style="padding:0 32px 12px;"><div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#9CA3AF;border-top:1px solid #E5E7EB;padding-top:16px;margin-bottom:14px;">Part 1 · General Competencies</div>${themeRows(generalResults)}</td></tr>
<tr><td style="padding:0 32px 12px;"><div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#9CA3AF;border-top:1px solid #E5E7EB;padding-top:16px;margin-bottom:14px;">Part 2 · Accounting Functional Competencies</div>${themeRows(functionalResults)}</td></tr>
<tr><td style="padding:0 32px 28px;"><div style="background:${BRAND.cinnabarTint};border-radius:10px;padding:16px 20px;"><div style="font-weight:700;color:${BRAND.cinnabar};font-size:13px;margin-bottom:5px;">Next steps</div><div style="font-size:13px;color:#5A2010;line-height:1.65;">Discuss these results in your next 1-to-1. Focus on where your self-assessment aligns with your manager's perspective — the gaps are the richest areas for feedback and development planning.</div></div></td></tr>
<tr><td style="background:#F9FAFB;padding:14px 32px;border-top:1px solid #E5E7EB;"><div style="font-size:11px;color:#9CA3AF;">saas.group · Career Framework · Accounting Job Family · ${completedDate}</div></td></tr>
    ...
    <div ref={topRef} style={{ maxWidth:620, margin:"0 auto", padding:"40px 20px 56px" }}>
      ...
      <div style={{ background:BRAND.cinnabarTint, border:"1px solid "+BRAND.cinnabar+"35", borderRadius:14, padding:"20px 22px", marginTop:24, marginBottom:8 }}>
        ...
        <div style={{ background:"#fff", border:"1px solid "+BRAND.cinnabar+"30", borderRadius:10, padding:"14px 16px", marginBottom:10 }}>
          ...
          <div style={{ fontSize:12.5, color:"#6B7280", lineHeight:1.6, marginBottom:12 }}>Opens a print-ready page. Use <strong>File → Print → Save as PDF</strong> (or <strong>Cmd+P</strong> / <strong>Ctrl+P</strong>), then attach to an email to your manager.</div>
          <button onClick={downloadPdf}
            style={{ width:"100%", padding:"11px 16px", fontSize:13.5, fontWeight:600, borderRadius:9, border:"none", background:BRAND.cinnabar, color:"#fff", cursor:"pointer", transition:"background 0.2s" }}
        ...
        <div style={{ background:"#fff", border:"1px solid "+BRAND.cinnabar+"30", borderRadius:10, padding:"14px 16px" }}>
          ...
          <ol style={{ margin:"0 0 12px", paddingLeft:18, fontSize:12.5, color:"#5A2010", lineHeight:1.9 }}>
            ...
            {managerEmail ? <li>Address it to <strong>{managerEmail}</strong></li> : <li>Address it to your manager's email</li>}
            <li>CC yourself at <strong>{memberEmail}</strong></li>
            <li>Hit Send</li>
          </ol>
          <button onClick={copyHtmlEmail}
            style={{ width:"100%", padding:"11px 16px", fontSize:13.5, fontWeight:600, borderRadius:9, border:"none", background:copied?"#16A34A":BRAND.indigo, color:"#fff", cursor:"pointer", transition:"background 0.25s", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}
      ...
      {!showRestartConfirm ? (
        <button onClick={() => setShowRestartConfirm(true)} style={{ width:"100%", marginTop:2, padding:"12px 20px", fontSize:13, fontWeight:600, borderRadius:10, border:"1px solid #E5E7EB", background:"#fff", color:"#9CA3AF", cursor:"pointer" }}>Start over</button>
      ) : (
...
export default function App() {
  ...
  }, [screen, qIdx, answers, memberName, memberEmail, managerEmail]);
